...
const getPostDate = (post: Post): Date | null => {
  ...
  if (typeof post.createdAt === 'object' && 'toDate' in post.createdAt && typeof post.createdAt.toDate === 'function') {
  ...
  <div class="calendar-root">
    ...
    >
      ...
      <div 
        ...
        class="calendar-cell"
        :class="{
          'other-month': currentView === 'month' && !isSameMonth(day, currentDate),
      ...
      >
        ...
        <div class="cell-posts">
          ...
          >
            <video v-if="post.mediaType === 'video'" :src="post.mediaUrl" class="entry-thumb" muted></video>
            <img v-else :src="post.mediaUrl" class="entry-thumb" />
            <h4>{{ post.title }}</h4>
          </div>
        </div>
      </div>
